	...
	<form action="organization/menuQuery.action" id="queryForm">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<div class="row">
					<div class="col-md-1" align="right">菜单名称</div>
					<div class="col-md-2">
						<input type="text" name="menuName" value="${menuName }" />
					</div>
					<div class="col-md-1" align="right">菜单URL</div>
					<div class="col-md-2">
						<input type="text" name="menuUrl" value="${menuUrl }" />
					</div>
					<div class="col-md-1">
						<button type="button" class="btn btn-primary" onclick="queryMenu()">查询</button>
					</div>
					<div class="col-md-1">
						<button type="button" class="btn btn-primary" onclick="addMenu()">添加</button>
					</div>
				</div>
			</div>
		</div>
							...
							<s:else>
								<s:iterator value="menuList" status="st">
									<tr>
										...
										<td><a href="javascript:viewMenu('<s:property value='id' />')"><s:property value='menuName' /></a></td>
										<td><s:property value='menuNamePath' /></td>
										<td><s:property value='menuUrl' /></td>
										<td><a href="javascript:deleteMenu('<s:property value='id' />')">删除</a></td>
									</tr>
								</s:iterator>
							</s:else>
						</tbody>
...
function deleteMenu(id) {
	var _url = "organization/delMenu.action";
	var params = $("#queryForm").serialize();
...
function viewMenu(id) {
	var _url = "organization/viewMenu.action?menuId=" + id;
	window.open(_url);
}

function addMenu(){
	var dId = document.getElementById("menuId").value;
	var _url = "organization/viewMenu.action?parentMenuId=" + dId;
	window.open(_url);
}
