	...
	<form action="organization/roleQuery.action" id="queryForm">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<div class="row">
					<div class="col-md-1" align="right">角色名</div>
					<div class="col-md-2">
						<input type="text" name="roleName" value="${roleName }" />
					</div>
					<div class="col-md-1">
						<button type="button" class="btn btn-primary" onclick="queryRole()">查询</button>
					</div>
					<div class="col-md-1">
						<button type="button" class="btn btn-primary" onclick="addRole()">添加角色</button>
					</div>
				</div>
			</div>
		</div>
							...
							<s:else>
								<s:iterator value="roleList" status="st">
									<tr>
										...
										<td><a href="javascript:viewRole('<s:property value='id' />')"><s:property value='roleName' /></a></td>
										<td><s:property value='dept.deptNamePath' /></td>
										<td><a href="javascript:deleteRole('<s:property value='id' />')">删除</a></td>
									</tr>
								</s:iterator>
							</s:else>
						</tbody>
...
function deleteRole(id) {
	var _url = "organization/delRole.action";
	var params = $("#queryForm").serialize();
	params = params + "&delRoleId=" + id;
	queryByAjax("roleManagerContainer", _url, "post", params);
}

function viewRole(id) {
	var _url = "organization/viewRole.action?roleId=" + id;
	window.open(_url);
}

function addRole(){
	var dId = document.getElementById("deptId").value;
	var _url = "organization/viewRole.action?deptId=" + dId;
	window.open(_url);
}
